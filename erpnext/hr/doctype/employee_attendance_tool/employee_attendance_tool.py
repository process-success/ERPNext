...
@frappe.whitelist()
def mark_single_employee_Cvrattendance(employee, employee_name,  date, status, company=None):
	attendance = frappe.new_doc("Crv_attendance")
	...
	if company:
		...
		attendance.company = frappe.db.get_value("Employee", employee, "company")
	attendance.submit()
	return "mark_single_employee_Cvrattendance"
...
@frappe.whitelist()
def mark_single_vineyard(name, address):
	attendance = frappe.new_doc("vineyards")
	#attendance.naming_series = series
	attendance.vineyard_name = name
	attendance.address = address
	attendance.submit()
	return "mark_single_vineyard"
